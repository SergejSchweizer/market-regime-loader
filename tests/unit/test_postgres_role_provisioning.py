...
def test_exact_endpoint_and_repository_role() -> None:
    ...
    assert POSTGRES_ROLE == "market-regime-loader"
...
def test_sql_is_least_privilege_and_schema_scoped() -> None:
    ...
    assert 'CREATE ROLE "market-regime-loader"' in sql
    for token in (
        "LOGIN PASSWORD",
        "NOSUPERUSER",
        "NOCREATEDB",
        ...
        assert token in sql
    assert POSTGRES_SCHEMAS == ("market_regime", "market_regime_sync")
    assert 'CREATE SCHEMA IF NOT EXISTS "market_regime"' in sql
    assert 'CREATE SCHEMA IF NOT EXISTS "market_regime_sync"' in sql
...
def test_sql_is_idempotent_and_fails_on_incompatible_existing_state() -> None:
    ...
    assert "existing market-regime-loader role has incompatible privileges" in sql
